...
def init_local_state() -> Expr:
    ...
    return Seq(
        ...
        App.localPut(Txn.sender(), LocalState.family_role, Int(0)),
    )
...
smart_asa_abi = Router(
    name="Smart ASA for Role Based Txn",
...
def pay_merchant(
    ...
    sender_is_family = App.localGet(sender, LocalInts.family_role) == Int(1)
...
def donation_transfer(
    ...
    sender_is_donor = App.localGet(sender, LocalInts.donor_role) == Int(1)
    sender_is_redcross_role = App.localGet(sender, LocalInts.redcross_role) == Int(1)
    receiver_is_redcross_role = App.localGet(receiver, LocalInts.redcross_role) == Int(1)
    receiver_is_family_role = App.localGet(receiver, LocalInts.family_role) == Int(1)

    return Seq(
        Assert(
            Or(
                sender_is_donor,
                sender_is_redcross_role
            ),
            asset_sent_is_smart_asa,
            Or(
                receiver_is_redcross_role, 
                receiver_is_family_role,
            ), 
        ),
        Assert(sender == Txn.sender()),
...
@smart_asa_abi.method
def set_family_role(account: abi.Account) -> Expr:
    return Seq(
        App.localPut(account.address(), LocalInts.family_role, Int(1))
    )
...
@smart_asa_abi.method
def get_is_family(address: abi.Address,*, output: abi.Bool) -> Expr:
    return Seq(
        # Effects
        output.set(App.localGet(address.get(),LocalState.family_role) == Int(1)),
    )
